...
class RepositoryControllerTest(
    ...
    @Autowired val githubAppService: GithubAppService
) {
    ...
    fun `should list repositories for user installations`() {
        ...
        coEvery { githubAppService.listRepositories(newInstall.id) }.returns(RepositoryList(1, repoList))
        coEvery { githubRestClient!!.listRepositories(any()) }.returns(
            RepositoryList(
                1,
                repoList
            )
